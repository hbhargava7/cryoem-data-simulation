 ...
 def main(args):
	...
	if args['type'] == 'temet':
		...
		for i, f in enumerate(F):
		    ...
		    offset = 0
		    if f.get_id()[1] in args['ringflip_residues']:
		    	offset = 180
...
if __name__ == "__main__":
	...
	parser.add_argument('--exclude_residues', help="indexes of residues to exclude (separate with spaces)", nargs='+', type=int, required=False)
	parser.add_argument('--ringflip_residues', help="indexes of residues to ringflip (separate with spaces)", nargs='+', type=int, required=False)
